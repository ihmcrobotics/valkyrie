      ...
      { // Arms
         ...
         { // Elbow
            ...
            if (elbow != null)
            {
               ...
               {
                  FrameCapsule3D forearmShape = new FrameCapsule3D(elbowFrame, 0.35 * modelScale, 0.06 * modelScale);
                  forearmShape.getPosition().set(-0.03, robotSide.negateIfRightSide(0.2), 0.0);
                  forearmShape.getPosition().scale(modelScale);
                  forearmShape.getAxis().set(Axis3D.Y);
                  collidables.add(new Collidable(elbow.getSuccessor(), collisionMask, collisionGroup, forearmShape));
               }
